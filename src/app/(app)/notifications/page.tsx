...
export default async function NotificationsPage({ searchParams }: PageProps) {
  ...
  const unreadOnly = typeof sp.filter === "string" && sp.filter === "unread" ? true : false;
  ...
  const unreadCount = unreadOnly ? items.length : items.filter((i) => !i.readAt).length;
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <CardDescription>
              {unreadOnly ? "Pouze nepřečtené." : "Všechny notifikace, nejnovější první."}
            </CardDescription>
          </div>
