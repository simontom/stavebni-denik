...
import { MAX_BATCH_BYTES, preparePhotoForUpload, PhotoClientPrepareError } from "./photo-client";
...
describe("photo-client unit tests", () => {
  ...
  it("throws PhotoClientPrepareError when file is empty (size 0)", async () => {
    ...
    await expect(preparePhotoForUpload(emptyFile)).rejects.toThrow("Soubor je prázdný.");
    await expect(preparePhotoForUpload(emptyFile)).rejects.toBeInstanceOf(PhotoClientPrepareError);
  });
});
