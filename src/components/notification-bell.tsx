    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger
        render={
          <button
            type="button"
            className="text-muted-foreground hover:bg-accent hover:text-foreground relative inline-flex size-9 items-center justify-center rounded-md"
      ...
      <DropdownMenuContent align="end" className="w-[min(20rem,90vw)] p-0">
        <div className="flex items-center justify-between border-b px-3 py-2 text-sm font-medium">
          <span>Notifikace</span>
          {unreadCount > 0 && (
            <button
              type="button"
              className="text-muted-foreground hover:text-foreground inline-flex items-center gap-1 text-xs font-normal disabled:opacity-50"
        ...
        {items.length === 0 ? (
          <p className="text-muted-foreground px-3 py-4 text-sm">Žádné notifikace.</p>
        ) : (
          <ul className="max-h-80 overflow-y-auto">
            {items.map((n) => (
              <li
                key={n.id}
                className={"border-b last:border-b-0 " + (n.readAt ? "" : "bg-muted/40")}
              >
                <button
                  type="button"
                  onClick={() => handleClick(n)}
                  disabled={pending}
                  className="hover:bg-accent grid w-full gap-0.5 px-3 py-2 text-left disabled:opacity-50"
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className={n.readAt ? "text-sm" : "text-sm font-medium"}>{n.title}</span>
                    {!n.readAt && <Check className="text-primary size-3" aria-label="nepřečtené" />}
                  </div>
                  ...
                  <div className="text-muted-foreground flex items-center justify-between text-[10px]">
                    ...
                    <button
                      ...
                      disabled={pending}
                      className="hover:text-destructive inline-flex items-center gap-1 disabled:opacity-50"
                      aria-label="Smazat"
                    >
